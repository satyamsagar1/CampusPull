...
const InterviewPYQSection = ({ pyqs, viewMode = 'grid', onEditClick }) => {
  ...
  const difficultyLevels = [
    ...
    { value: 'beginner', label: 'Easy', color: 'text-green-600 bg-green-100 border-green-200' },
    { value: 'intermediate', label: 'Medium', color: 'text-yellow-600 bg-yellow-100 border-yellow-200' },
    { value: 'advanced', label: 'Hard', color: 'text-red-600 bg-red-100 border-red-200' }
  ];
  ...
  const getDifficultyLabel = (val) => difficultyLevels.find(l => l.value === val)?.label || val;
    ...
    <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {companyFilters?.map((company) => (
                ...
                >
                  {company?.label}
                  <span className={`ml-1.5 text-[10px] ${selectedCompany === company?.value ? 'text-indigo-200' : 'text-gray-400'}`}>
                    {company?.count}
                  </span>
                </button>
              ))}
      ...
      <div className={viewMode === 'grid' ? "grid grid-cols-1 lg:grid-cols-2 gap-6" : "flex flex-col gap-4"}>
        {filteredPYQs?.map((pyq) => {
          ...
          if (viewMode === 'list') {
             return (
              <div key={pyq._id} className="group bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl p-5 hover:shadow-xl transition-all duration-300">
                <div className="flex flex-col sm:flex-row items-start gap-5">
                   ...
                   <div className="flex-shrink-0 w-16 h-16 bg-white rounded-xl shadow-sm border border-gray-100 flex items-center justify-center overflow-hidden p-2 group-hover:scale-105 transition-transform">
                      {pyq.thumbnail ? (
                        <Image src={pyq.thumbnail} alt={pyq.company} className="w-full h-full object-contain" />
                      ) : (
                   ...
                   <div className="flex-1 w-full">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                           <h3 className="font-bold text-gray-800 text-lg group-hover:text-indigo-700 transition-colors">{pyq.company}</h3>
                           <p className="text-gray-500 text-sm font-medium">{pyq.title}</p>
                        </div>
                        <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${getDifficultyColor(pyq?.difficulty)}`}>
                            {getDifficultyLabel(pyq?.difficulty)}
                        </div>
                      </div>

                      <p className="text-gray-500 text-sm mb-3 line-clamp-2">{pyq.description || "No description provided."}</p>

                      {/* Tags */}
                      <div className="flex flex-wrap gap-2 mb-4">
                        {pyq.tags?.slice(0, 5).map((tag, i) => (
                           <span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] rounded-md border border-indigo-100 font-semibold">
                              {tag}
                           </span>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-gray-100/50">
                         <div className="flex items-center gap-2">
                            <Image src={pyq?.uploadedBy?.avatar} className="w-5 h-5 rounded-full" />
                            <span className="text-xs text-gray-500">by {pyq?.uploadedBy?.name}</span>
                         </div>
                         <div className="flex gap-2">
                            {user?._id === pyq?.uploadedBy?._id && (
                              <Button variant="outline" size="sm" iconName="Edit" onClick={() => onEditClick(pyq)}>
                                Edit
                              </Button>
                            )}
                            <Button 
          ...
          // ==========================================
          return (
            <div key={pyq?._id} className="group bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl p-6 hover:shadow-xl transition-all duration-300 flex flex-col h-full">
              {/* Header */}
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-3">
                  <div className="w-14 h-14 rounded-xl bg-white shadow-sm border border-gray-100 p-2 flex-shrink-0 flex items-center justify-center group-hover:scale-105 transition-transform">
                    {pyq.thumbnail ? (
                      <Image
                        src={pyq.thumbnail}
                        alt={pyq.company}
      ...
      {filteredPYQs?.length === 0 && (
        <div className="text-center py-20 bg-white/40 backdrop-blur-md rounded-3xl border border-dashed border-gray-300">
          <Icon name="Search" size={48} color="#9ca3af" className="mx-auto mb-4 opacity-50" />
          <h3 className="text-lg font-bold text-gray-700 mb-1">No PYQs Found</h3>
